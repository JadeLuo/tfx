...
@beam.typehints.with_output_types(Tuple[Text, tf.train.Example])
class _RowToContextFeatureAndExample(beam.DoFn):
  ...
  def process(
      self, instance: Dict[Text,
                           Any]) -> Iterable[Tuple[Text, tf.train.Example]]:
    context_feature = dict((k, instance[k])
                           for k in instance.keys()
                           if k in self._context_feature_fields)
    context_feature_key = json_utils.dumps(context_feature)
...
def _ConvertContextAndExamplesToElwc(
    context_feature_and_examples: Tuple[Text, List[tf.train.Example]],
    type_map: Dict[Text, Text]) -> input_pb2.ExampleListWithContext:
  """Convert context feature and examples to ELWC."""
  context_feature, examples = context_feature_and_examples
  context_feature_dict = json_utils.loads(context_feature)
  context_feature_proto = utils.row_to_example(type_map, context_feature_dict)
...
def _BigQueryToElwc(pipeline: beam.Pipeline, exec_properties: Dict[Text, Any],
  ...
  return (
      ...
      | 'ConvertContextAndExamplesToElwc' >> beam.Map(
          _ConvertContextAndExamplesToElwc, type_map))
